...
function renderDeck() {
  ...
  const visibleCards = state.flashcardsExpanded ? cards : cards.slice(0, 12);
  ...
  if (cards.length > 12) {
    ...
    toggle.className = "flashcard-toggle";
    toggle.type = "button";
    toggle.textContent = state.flashcardsExpanded ? "Thu gọn flashcards" : `Xem tất cả ${cards.length} flashcards`;
    toggle.addEventListener("click", () => {
      state.flashcardsExpanded = !state.flashcardsExpanded;
      renderDeck();
      document.querySelector("#deck").scrollIntoView({ behavior: "smooth", block: "start" });
    });
    deckGrid.appendChild(toggle);
  }
}
